...
export abstract class AbstractRestService<T> {
    ...
    list(url: string, options ?: object): Promise<any> {
        return this.http.get<T[]>(url, options).toPromise();
    }

    get(url: string, id: number, options ?: object): Promise<any> {
        return this.http.get<T>(`${url}/${id}`, options).toPromise();
    }

    create(url: string, object: T, options ?: object): Promise<any> {
        return this.http.post<T>(url, object, options).toPromise();
    }

    put(url: string, id: number, object: T, options ?: object): Promise<any> {
